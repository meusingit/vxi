...
public:
  ...
  VXIlogResult Init(const VXIchar   *filename,
  ...
  VXIlogResult WriteEntry(const SBlogLogEntry& entry, 
...
VXIlogResult
SBlogLogStream::Init(const VXIchar   *filename,
         const VXIchar   *fileMimeType,
         VXIint32         maxLogSizeBytes,
         const VXIchar   *logContentDir,
         VXIint32         maxContentDirSize,
...
VXIlogResult
SBlogLogStream::WriteEntry(const SBlogLogEntry &entry, VXIbool logToStdout)
{
  VXIlogResult rc = VXIlog_RESULT_SUCCESS;
  ...
  if (logfilename_ != NULL) {
    ...
    rc = Open(entry.size());
    if (rc == VXIlog_RESULT_SUCCESS) {
      int nwrite = fwrite(fileBuf, 1, nFileBytes, logFp_);
      if (nwrite == 0) {
...
)
{
  ...
  entry.Append(keys, values);
  entry.Terminate();
  g_LogStream->WriteEntry(entry, FALSE);
}
