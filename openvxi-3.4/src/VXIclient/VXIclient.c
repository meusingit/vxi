...
VXIplatformResult VXIplatformInit(VXIMap *configArgs, VXIunsigned *nbChannels)
{
  ...
  /* Initialize log */
  {
    ...
    if (!errorMapFiles) {
      ...
      logResult = SBlogListenerInitEx3(lfname, lfmime, lmsize*1024*1000, ldname, 
   ...
   */
  {
    ...
    VXIint32 runtimeSize = 16384000;
    VXIint32 contextSize = 131072;
    VXIint32 maxBranches = 100000;
    diagLogBase          = 0;
               ...
               VXIplatform *platform)
{
  ...
  */
  ...
  /* Put the telephony map into the session ECMAScript so they will be
     referenced as session.telephone.*. */
  if (platform->telephonyProps) {
    const VXIValue *val;
    val = VXIMapGetProperty(platform->telephonyProps, L"ani");
    if ((val) && (VXIValueGetType(val) == VALUE_STRING))
      wcscpy(ani, VXIStringCStr((const VXIString *) val));

    val = VXIMapGetProperty(platform->telephonyProps, L"dnis");
    if ((val) && (VXIValueGetType(val) == VALUE_STRING))
      wcscpy(dnis, VXIStringCStr((const VXIString *) val));
    
    /* Create a ECMAScript for telephony properties in session scope */
    SWIswprintf(tempScript, STRINGLEN, 
