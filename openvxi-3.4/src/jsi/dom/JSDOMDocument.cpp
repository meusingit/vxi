...
JSObject *JSDOMDocument::JSInit(JSContext *cx, JSObject *obj) {
   ...
   jsval oldobj;
   if (JS_TRUE == JS_LookupProperty(cx, obj, JSDOMDocument::_jsClass.name, &oldobj) && JSVAL_IS_OBJECT(oldobj))
      return JSVAL_TO_OBJECT(oldobj);

   JSObject *newobj = JS_InitClass(cx, obj, NULL, &JSDOMDocument::_jsClass,
                                   NULL, 0,
                                   JSDOMDocument::_JSPropertySpec, JSDOMDocument::_JSFunctionSpec,
                                   NULL, NULL);
